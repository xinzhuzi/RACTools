...
#import "Signal_Class_VC.h"
#import "RACTestView.h"

@interface Signal_Class_VC ()

@end

@implementation Signal_Class_VC
...
- (void)textSignalReplaceDelegate{
    RACTestView *dView = [[RACTestView alloc]initWithFrame:CGRectMake(0, 64, Screen_W, Screen_H-64)];
    dView.delegateSignal = [RACSubject subject];
    [dView.delegateSignal subscribeNext:^(id x) {
        NSLog(@"点击了button,传递过来一个button,这个不属于真正的代理协议:%@",x);
    }];
    [self.view addSubview:dView];
    
    RACSignal *pSignal = [dView rac_signalForSelector:@selector(testButtonClickDelegate:) fromProtocol:@protocol(RACTestViewDelegate)];
    [pSignal subscribeNext:^(id x) {
